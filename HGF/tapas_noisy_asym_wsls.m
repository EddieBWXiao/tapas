function [logp, yhat, res] = tapas_noisy_asym_wsls(r, infStates, ptrans)
% Calculates the log-probability of response y=1 for a noisy win-stay lose-shift model
% asymmetric so epsilon different for post-reward vs not
% note: treatment of missing data is problematic
% 
...
logitepsilsonwin = ptrans(1);
logitepsilsonloss = ptrans(2);
epsilon_win = tapas_sgm(logitepsilsonwin, 1);
epsilon_loss = tapas_sgm(logitepsilsonloss, 1);
    ...
    if t == 1
        ...
        if ismember(t-1, r.irr)
            ...
            if u_full(t-1) == 1
                ...
                p_y1(t) = 1 - (epsilon_win+epsilon_loss)/2;
            else
                % if previously 0 was rewarded, go 0
                p_y1(t) = (epsilon_win+epsilon_loss)/2;
            end
            ...
            if prevReward == 1
                ...
                if prevChoice == 1
                    p_y1(t) = 1 - epsilon_win/2;  % stay with action 1
                else
                    p_y1(t) = epsilon_win/2;      % stay with action 0 (low prob of switching to 1)
                end
            else
                % lose-shift: If prevChoice was 1, shift with high probability
                if prevChoice == 1
                    p_y1(t) = epsilon_loss/2;      % shift from action 1 to 0 (low prob of staying with 1)
                else
                    p_y1(t) = 1 - epsilon_loss/2;  % shift from action 0 to 1
                end                
            end
