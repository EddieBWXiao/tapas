function [y, prob] = tapas_sgm_wsls_cbstick_sim(r, infStates, p)
% Simulates observations from a Bernoulli distribution with choice bias and
% stickiness  and wsls bias
% 
...
wsbonus = p(4);   % Win-stay bonus
lsbonus = p(5);   % Lose-shift bonus
...
for t = 1:n
    ...
    if t == 1
        ...
        if prevReward && (y(t-1) == 1)
            y1win = 1;
        elseif prevReward && (y(t-1) == 0)
            y1win = -1;
        elseif (~prevReward) && (y(t-1) == 1)
            y1loss = 1;
        elseif (~prevReward) && (y(t-1) == 0)
            y1loss = -1;
        end
    ...
    prob(t) = tapas_sgm(ze * (x_real + bias + stickiness * choice_trace_diff + wsbonus*y1win + lsbonus*y1loss), 1);
