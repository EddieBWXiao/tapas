function [y, prob] = tapas_noisy_wsls_sim(r, infStates, p)
% Simulates observations from a noisy win-stay lose-shift model
...
% Subsequent trials: apply win-stay lose-shift strategy with noise
for t = 2:n
    ...
    if prevReward == 1
        ...
        if prevChoice == 1
            prob(t) = 1 - epsilon/2;  % stay with action 1
        else
            prob(t) = epsilon/2;      % stay with action 0 (low prob of switching to 1)
        end
    else
        % lose-shift: If prevChoice was 1, shift with high probability
        if prevChoice == 1
            prob(t) = epsilon/2;      % shift from action 1 to 0 (low prob of staying with 1)
        else
            prob(t) = 1 - epsilon/2;  % shift from action 0 to 1
        end                
    end
