function [logp, yhat, res] = tapas_sgm_wsls_cbstick(r, infStates, ptrans)
% Calculates the log-probability of response y=1 for sigmoid response
% function with a choice bias term and choice stickiness term and wsls bias
% 
% --------------------------------------------------------------------------------------------------
...
% additional definition, necessary for win-stay lose-shift
isRewarded = y == u;
prevRewarded = [false; isRewarded(1:end-1)]; %had to use false for initial; it will be zeroed out later
%prevRewarded([false; ismember((1:n-1)', r.irr)]) = NaN; %REMOVED:
    %NaN cannot into logical, so I will rely on the definition of y1win and
    %y1loss to filter these out
...
% wsls: dependency on rewarded or not
y1win = double(prevRewarded & (prev_choice == 1));
y1win(prevRewarded & (prev_choice == 0)) = -1; % so that win-stay bonus goes to the OTHER choice
y1win(isnan(prev_choice)) = 0; %irregular trials go 0; IMPORTANT: this is where they are handelled
y1loss = double((~prevRewarded) & (prev_choice == 1));
y1loss((~prevRewarded) & (prev_choice == 0)) = -1; % should make current choice more likely if the alt not rewarded
y1loss(isnan(prev_choice)) = 0; %irregular trials go 0;  IMPORTANT: this is where they are handelled

% the sigmoid function:
p_y1 = tapas_sgm(ze * (x_real + bias + stickiness * choice_trace_diff + wsbonus*y1win + lsbonus*y1loss), 1);
